...
const AuditInput = z.object({
    ...
    categories: z.string().optional().describe("扫描类别，逗号分隔。可选: injection, access_control, crypto, misconfiguration, secrets, misc。留空=全量"),
});
// ---------------------------------------------------------------------------
// 配置
// ---------------------------------------------------------------------------
const CONFIG = {
    title: "全栈安全审计",
    description: "对目标项目执行 OWASP Top 10 + CWE Top 25 全类别安全审计。" +
...
// 注册
// ---------------------------------------------------------------------------
export function registerSecurityAudit(server) {
    server.registerTool("security_audit", CONFIG, async (args) => {
